"""Tests for training a Bayesian linear head on cached feature bundles."""
...
from scripts.training.train_bayesian_feature_head import run_bayesian_feature_head_training
...
def test_run_bayesian_feature_head_training_writes_expected_outputs(tmp_path: Path) -> None:
    ...
    assert metrics_json["mc_samples_train"] == 2
    assert metrics_json["mc_samples_eval"] == 4
    assert metrics_json["prior_std"] == 1.0
    assert metrics_json["kl_weight"] == pytest.approx(0.25)
    ...
    assert "epoch_result" in best_metrics_json
    ...
    assert config_json["kl_weight"] == pytest.approx(0.25)
...
def test_run_bayesian_feature_head_training_supports_best_epoch_selection_metric(
    tmp_path: Path,
) -> None:
    train_features_path = tmp_path / "train_features.pt"
    val_features_path = tmp_path / "val_features.pt"
...
def test_run_bayesian_feature_head_training_rejects_invalid_mc_samples(
    tmp_path: Path,
) -> None:
    train_features_path = tmp_path / "train_features.pt"
    val_features_path = tmp_path / "val_features.pt"
    ...
    with pytest.raises(ValueError, match="mc_samples_eval must be positive"):
        run_bayesian_feature_head_training(
            train_features=train_features_path,
            val_features=val_features_path,
            output_dir=tmp_path / "outputs",
            num_classes=2,
