"""Tests for training a linear head on cached feature bundles."""
...
from scripts.training.train_feature_head import run_feature_head_training
...
def test_run_feature_head_training_writes_expected_outputs(tmp_path: Path) -> None:
...
def test_run_feature_head_training_rejects_mismatched_feature_dimensions(
    tmp_path: Path,
) -> None:
    train_features_path = tmp_path / "train_features.pt"
    val_features_path = tmp_path / "val_features.pt"
    ...
    try:
        ...
        assert "matching feature dimensions" in str(error)
    else:
        raise AssertionError("Expected mismatched feature dimensions to raise ValueError.")
