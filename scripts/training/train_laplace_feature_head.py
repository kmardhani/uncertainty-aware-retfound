"""Train a deterministic linear head and fit a diagonal Laplace posterior."""
...
def _predictive_statistics(
    ...
    """Compute Monte Carlo predictive statistics from a Laplace posterior."""

    logits_stack = sample_laplace_logits(
        posterior=posterior,
        features=features,
        num_samples=mc_samples_eval,
        device=device,
    )
...
def _evaluate_laplace_model(
    ...
    """Evaluate a diagonal Laplace posterior with Monte Carlo predictive averaging."""
    ...
    with torch.no_grad():
        for batch in dataloader_iterator:
            ...
            batch_loss = float(
                (-torch.log(torch.clamp(selected_probabilities, min=1e-12))).mean().item()
            )
...
def _write_validation_predictions_csv(
    ...
    """Write Laplace predictive statistics to CSV."""
...
def run_laplace_feature_head_training(
    ...
    train_eval_dataloader = create_dataloader(
        train_dataset,
        batch_size=batch_size,
        shuffle=False,
        num_workers=0,
    ...
    final_posterior: DiagonalLinearLaplacePosterior | None = None
    ...
    for epoch_index in range(epochs):
        ...
        validation_result = _evaluate_laplace_model(
            posterior=posterior,
    ...
    if final_validation_result is None or final_posterior is None:
        raise RuntimeError("Final Laplace validation result was not produced during training.")
    ...
    _write_validation_predictions_csv(final_validation_result, validation_predictions_path)
    best_validation_predictions_path = output_dir_path / "best_validation_predictions.csv"
    _write_validation_predictions_csv(best_validation_result, best_validation_predictions_path)
    ...
    laplace_posterior_path = output_dir_path / "laplace_posterior.pt"
    torch.save(final_posterior.to_dict(), laplace_posterior_path)
    ...
    metrics = {
        **config,
        ...
        "laplace_posterior_path": str(laplace_posterior_path),
...
def parse_args() -> argparse.Namespace:
    """Parse cached-feature Laplace-head training CLI arguments."""

    parser = argparse.ArgumentParser(
        description="Train a deterministic linear head and fit a diagonal Laplace posterior."
    )
    ...
    parser.add_argument("--mc-samples-eval", type=int, default=30)
    parser.add_argument(
        "--selection-metric",
        type=str,
...
def main() -> None:
    """Run the cached-feature Laplace-head training CLI."""
